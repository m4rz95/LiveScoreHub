...
type Row = { teamId:number; teamName:string; played:number; win:number; draw:number; loss:number; gf:number; ga:number; gd:number; points:number, last5: string[]; // ["W","D","L","W","-"]
 }

export default function StandingsPage() {
  const [rows, setRows] = useState<Row[]>([])
  async function load(){ const res = await fetch('/api/standings', { cache:'no-store' }); setRows(await res.json()) }
  useEffect(()=>{ load() },[])
  return (
   <Card>
